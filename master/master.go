...
import (
	...
	. "github.com/rahafrouz/fogflow/common/communicator"
...
func (master *Master) SelectWorkerAHP() string {
	...
	var lowestUtility float64 = math.MaxFloat64
	var utility float64
	DEBUG.Printf("Worker Selection according to ahp\n %v\n", master.edgeUtilization)
	for _, worker := range master.workers {
		...
		DEBUG.Printf("Utility of Worker %v is: %v", worker.WID, utility)
		if utility <= lowestUtility {
			selectedWorkerID = worker.WID
			lowestUtility = utility
			DEBUG.Printf("Choosing the best worker: ", worker.WID, "with utility: ", utility)
		}

	}
	DEBUG.Printf("Worker Selected: %v\n", selectedWorkerID)
